...
def backup_db():
    remote_prefix = '{}'.format(os.environ['PGBACKUPDIR'])
    subprocess.run(['rclone', '--config', '/notfound', 'mkdir', f'remote:{remote_prefix}'], check=True)
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        remote_folder='{}/{}'.format(remote_prefix,now.strftime('%Y/%m/%d'))
        for database in databases:
            ...
            # rclone moveto /tmp/backup "remote:/mongobackup/$BACKUP_DIR/$(date +%Y-%m-%d_%H-%M-%S).dump"
            subprocess.run(['rclone', '-v', '--config', '/notfound', 'moveto', output, f'remote:{remote_folder}/{ts}/{database}.sql'], check=True)
